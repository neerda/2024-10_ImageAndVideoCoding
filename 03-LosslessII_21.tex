...
\begin{frame}{Conditional coding not always benficial}
\loud{Given distribution: }
\bit
\item Consider sources $X$ and $Y$ with alphabets $\mathcal{A}_X=\{0,1\}$ and $\mathcal{A}_Y=\{0,1,2\}$. 
\item Assume that the joint probability $p_{X,Y}$ is given as
...
%Let $X, Y, Z$ be random variables on a discrete probabilty space $\Omega$. 
%Then one has 
%\begin{align*}
%H(Z|X,Y)\leq H(Z|Y). 
%\end{align*}
...
\end{align*}
\item \loud{Already proved:} Sequence $H(X_i|X_{i-1},\dots,X_1)$ converges to $H'(\mathsf{X})$.  
