...
\item Well working iterative schemes for computation of KLT which are convergent:
\bit 
\item QR-method 
\item Jacobi method
\eit
...
\eit
\end{proposition}
\loud{\iarrow For a given source $X$, KLT computes best low-rank approximation of $X$.}
\end{frame}

\begin{frame}\frametitle{Proof of optimal energy compaction of KLT for $k=1$.}
\begin{itemize}
\item For $k=1$, one needs to find $v_1\in\mathbb{R}^N$ which solves the constraint optimization problem
...
\begin{align*}
\mathbb{E}(\dist(X,W)^2)\geq \mathbb{E}(\dist(X,V)^2).
\end{align*}
\end{itemize}
\qed
\end{frame}
...
\captionsetup{labelformat=empty}
\caption{Original handwritten digit (left), and its representation by using only a subset $v_1,\dots,v_M$ of the principal components. If $x\in\mathbb{R}^{784}$, the $M$-th picture
represents
