...
\begin{theorem}[Rate distortion funciont of a binary source]
...
\bit
\item A function $f:\Omega\to\mathbb{R}$ is called \loud{measurable} if 
$f^{-1}(A)\in\Sigma$ for all $A\in\mathfrak{B}(\mathbb{R})$. 
\item A measurable function $f: \Omega\to\mathbb{R}$ is called \loud{simple function} if 
...
\]
\item [\iarrow] Obvious definition of \loud{integral for simple functions $f$}:
\begin{align*}
...
\end{align*}
\item \loud{Distribution of $X$}: The probability measure $\mu_X$ on $\mathfrak{B}(\mathbb{R}^n)$ defined as 
\begin{align*}
\mu_X(A):=\mu(X^{-1}(A)).
\end{align*}
\item \loud{Cumulative distribution $F_X$ of $X$}: The function $F_X:\mathbb{R}^d\to [0,1]$ defined as
\begin{align*}
...
\loud{Setting of this lecture}
\bit
\item A random varible is also called \loud{source}. 
...
\bit
\item A random variable $X:\Omega\to\mathbb{R}^n$ is called \loud{absolutely continuous} if Borel sets of Lebesgue measure zero have zero probabilty 
under its distribution:
\begin{align*}
\mu_X(A)=0,\quad \text{$\forall A\in\mathfrak{B}(\mathbb{R}^n)$ with $\lambda_n(A)=0$}.
\end{align*} 
\item By \loud{Radon-Nikodym theorem}: Random variable $X$ is absolutely continuous if and only if
...
\item Function $f_X$ from \eqref{DefEqDensity} is called the \loud{density of $X$}. Unique up to set of measure zero. 
